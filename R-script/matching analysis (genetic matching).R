...
#EQQ plot's
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# ----
plot(m_out_cart1, type = "qq", interactive = FALSE,
...
stratification_function <- function(matchit_object){
  matched.dataset <- match.data(matchit_object)
  matched.dataset$strata <- cut(matched.dataset$propensity_score,
                               quantile(matched.dataset$propensity_score),
                               include.lowest=TRUE,
                               labels=FALSE)
...
# Stratify matched datasets 
# logit

stratifiedMatch_logit1<- stratification_function(m_out_logit1)
stratifiedMatch_logit2 <- stratification_function(m_out_logit2)
stratifiedMatch_logit3 <- stratification_function(m_out_logit3)
stratifiedMatch_logit4 <- stratification_function(m_out_logit4)

# Examine common support (cs) - no strata should have 0 treated or 0 control units

xtabs(~stratifiedMatch_logit1$treat+stratifiedMatch_logit1$strata)  # Treated and control counts
xtabs(~stratifiedMatch_logit2$treat+stratifiedMatch_logit2$strata)  # Treated and control counts
xtabs(~stratifiedMatch_logit3$treat+stratifiedMatch_logit3$strata)  # Treated and control counts
xtabs(~stratifiedMatch_logit4$treat+stratifiedMatch_logit4$strata)  # Treated and control counts



# Further diagnosis of covariate balance 

summary_table <- function(dataset){
  categorical_vars <- c('black','hispanic','married','nodegree','strata')
  nonnromal_variables <- c("re75","age")
  summaryTable <- CreateTableOne(vars = c('treat', 'age', 'education.',
                                               'black','hispanic','married','nodegree',
                                               're75','strata'), 
                                      data = dataset,
                                      argsNonNormal = nonnromal_variables,
                                      factorVars = categorical_vars,
                                      strata = c('treat','strata'))
  
  return(summaryTab_logit1)
}


print(summary_table(stratifiedMatch_logit2),exact = "stage", quote = TRUE, noSpaces = TRUE) 
